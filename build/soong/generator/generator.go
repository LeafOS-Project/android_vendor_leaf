...
func init() {
	android.RegisterModuleType("leaf_generator", GeneratorFactory)
}
...
func (g *Module) GenerateAndroidBuildActions(ctx android.ModuleContext) {
	...
	if depRoot == "" {
		...
		depRoot = leafExpandVariables(ctx, depRoot)
	}

	// Glob dep_files property
	for _, dep_file := range g.properties.Dep_files {
		dep_file = leafExpandVariables(ctx, dep_file)
	...
	cmd := leafExpandVariables(ctx, String(g.properties.Cmd))
