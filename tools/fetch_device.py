...
with open(f'{topdir}/.repo/local_manifests/{family}.xml', 'w') as local_manifest:
	...
	for repo in repositories:
		...
		if 'fetch' in repo:
			...
			if repo_fetch not in ElementTree.tostring(lm).decode():
				remote = ElementTree.Element("remote", attrib = { "name": repo_fetch, "fetch": repo_fetch })
				lm.append(remote)
